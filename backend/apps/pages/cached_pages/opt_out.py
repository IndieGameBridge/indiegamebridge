...
class OptOutPageBuilder(BaseCachedPageBuilder):
    ...
    def build_content(self) -> dict:
        return {
            ...
            "not_logged_in": {
                "prompt_content": f"Click below to log in with your Twitch account so we can verify your Twitch ID. We will then remove all data tied to it and exclude it from future collection. The removed data cannot be restored.",
                "login_btn": f"Log in with Twitch to verify your Twitch ID",
                "verifying": f"Redirecting you to Twitch to verify your Twitch ID…",
            },
            "logged_in": {
                "prompt_content": f"Click below to confirm. All data tied to your Twitch ID will be removed automatically and excluded from future collection. The removed data cannot be restored. To re-enable future collection later, log back in and turn tracking back on in your account settings. Your data may still appear on public pages for up to an hour due to cache latency.",
                "optout_btn": f"Opt Out",
                "optout_btn_pending": f"Opting you out…",
            },
